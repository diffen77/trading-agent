...
test(
  'operational status query fails closed against schema 52',
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(status.schema_version, 52)
...
test(
  'authorized dashboard quote view executes against schema 52',
  { skip: !TEST_DATABASE_URL },
  async () => {
    const pool = new pg.Pool({
      connectionString: TEST_DATABASE_URL,
